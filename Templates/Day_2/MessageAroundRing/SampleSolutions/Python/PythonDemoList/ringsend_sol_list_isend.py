...
def exchange(comm, up_send, dn_send, up_neigh, dn_neigh):
    ...
    up_req = comm.isend(up_send, dest=up_neigh, tag=1)
    dn_req = comm.isend(dn_send, dest=dn_neigh, tag=2)

    dn_recv = comm.recv(source=dn_neigh, tag=1)
    up_recv = comm.recv(source=up_neigh, tag=2)
